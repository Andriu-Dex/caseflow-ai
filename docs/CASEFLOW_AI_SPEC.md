...
- El motor determinístico separa modelo estructurado, generación de fuente, validación y render. ER usa `MERMAID_ER`; UML de casos de uso usa `PLANTUML` porque conserva actores, asociaciones y límite de sistema sin degradarlos a flowchart. El motor no ejecuta Mermaid, PlantUML, shell ni texto de IA: `caseflow-svg-v1` valida exclusivamente la fuente generada internamente y produce SVG escapado, acotado y regenerable sin red ni servicio adicional.
